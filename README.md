...
- `NetService`, `NetworkSetup`, & `PortInspector` — manages macOS network routing modes (`net status`, `net system-proxy on|off`, `net tun on|off`, `net proxy-mode on|off`, `net off`) with mutual exclusivity, `networksetup` system proxy integration, port conflict detection via `lsof` with PID/process attribution, and privilege safety handling. utun collision detection uses `ifconfig -a` and requires the interface to be `UP` **and** carry a non-link-local address, not just be present by name — a real-hardware false-positive fix, see CHANGELOG #27. Tested in `Tests/mihomo-cliTests/NetServiceTests.swift`, `Tests/mihomo-cliTests/NetworkSetupTests.swift`, and `Tests/mihomo-cliTests/PortInspectorTests.swift`.
