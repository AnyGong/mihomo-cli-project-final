...
## 29. New feature — macOS menu bar icon (`mihomo-menubar`)

User requested a persistent macOS menu bar icon with: Show Main Window (opens `http://127.0.0.1:9090`); an Outbound Mode submenu (Direct/Global/Rule, single-select); Set as System Proxy and Enhanced Mode (TUN mode) toggles with checkmarks; a Switch Configuration submenu (single-select); Reload Configuration; and a Launch at Login toggle.

**This is an explicit, narrow exception to AI.md §1's "No GUI, ever."** Full rationale and scope boundary written up in the new `docs/mihomo_menubar_spec.md` §0 — nothing else about the project's no-distribution/no-packaging/single-machine scope changed; only the menu bar icon itself is in scope, and it renders no UI of its own beyond the menu (Show Main Window opens the *existing* dashboard in the default browser rather than embedding anything).

**What was built:**

- `Package.swift`: added a second `.executableTarget`, `mihomo-menubar`, with no dependencies beyond system frameworks (AppKit). Deliberately does **not** depend on the `mihomo-cli` target — see architecture rationale below.
- `Sources/mihomo-menubar/CLILocator.swift`: resolves which `mihomo-cli` binary to drive (`MIHOMO_CLI_PATH` env override → sibling binary next to this app's own executable → `/usr/local/bin/mihomo-cli`).
- `Sources/mihomo-menubar/CLIBridge.swift`: `Process`-based wrapper that runs `mihomo-cli` subcommands and captures stdout/stderr/exit code; a `runJSON` convenience for `--json` read commands.
- `Sources/mihomo-menubar/MenuBarState.swift`: assembles a `MenuBarSnapshot` (outbound mode, network mode, subscription list) from three concurrent `mihomo-cli ... --json` calls (`mode status`, `net status`, `sub list`).
- `Sources/mihomo-menubar/LoginItemAgent.swift`: Launch-at-Login via a second, independent `launchd` agent (`~/Library/LaunchAgents/com.mihomo-cli.menubar.plist`, `RunAtLoad` only, no `KeepAlive`), reusing the `bootstrap`/`bootout` pattern already established in `Support/LaunchdAgent.swift` rather than a new mechanism, and deliberately not `SMAppService` (which requires an `.app` bundle).
- `Sources/mihomo-menubar/StatusItemController.swift`: builds the `NSStatusItem`/`NSMenu`, polls state every 5s and on menu-open, and maps every menu action to a `mihomo-cli` invocation (see the full mapping table in the spec doc); failures show the CLI's own `error: ...` text verbatim in an `NSAlert`.
- `Sources/mihomo-menubar/AppDelegate.swift` + `main.swift`: sets `NSApp.setActivationPolicy(.accessory)` at runtime (no Dock icon, no `.app` bundle/Info.plist needed) and starts the run loop.

**Architecture decision — subprocess, not shared library:** the menu bar app never calls into `ModeService`/`NetService`/`SubscriptionService` in-process. Every action shells out to the already-built, already-tested `mihomo-cli` binary, the same way a person would from a terminal — so locking, the atomic switch workflow, rollback, and liveness checks continue to run in exactly one place. Refactoring the existing single executable target into a shared library was considered and rejected as unnecessary risk/complexity for a single-user local tool where subprocess latency doesn't matter (AI.md §3's "not speculative generality" principle).

**Command mapping (see `docs/mihomo_menubar_spec.md` §2.5 for full detail and rationale):** Outbound Mode → `mode direct|global|rule` (`--yes` on direct/global only, `rule` has no such flag); System Proxy → `net system-proxy on --yes|off`; TUN → `net tun on --yes|off`; Switch Configuration → `sub use <name>`; Reload Configuration → `restart` (no dedicated "reload" command exists yet — `restart` is the existing tested atomic stop/start, which re-reads the active subscription).

**Not built:** any settings UI, notifications, or handling for the two binaries drifting apart in `--json` shape — all judged out of scope for a single-user tool built/updated as one package; see spec doc §5.

**Testing:** none yet. `NSStatusItem`/`NSMenu` construction isn't meaningfully unit-testable the way the rest of this codebase is; `CLILocator`, `CLIBridge`, and `LoginItemAgent` are the pieces with real logic and are the best candidates if test coverage is wanted later. **Not compiled or run** — written in a sandbox with no Swift/AppKit toolchain available (same caveat the whole original scaffold carried in CHANGELOG entry 6 / AI.md §4 item #6). `swift build` on the Mac Mini, followed by manually exercising each menu item against a running kernel, is required before trusting this.

## 30. Bugfix — "Set as System Proxy" from the menu bar: `error: invalid selection — selected index out of range`

**Reported symptom:** clicking "Set as System Proxy" in `mihomo-menubar` failed with `Couldn't enable system proxy — error: invalid selection - selectedindex out of range`, instead of either succeeding or showing the expected "multiple active network services" message.

**Root cause:** `CLIBridge.run()` spawned the `mihomo-cli` subprocess without setting `process.standardInput`, so it inherited whatever stdin the menu bar app itself had — a real TTY, when the app is launched directly from a Terminal session (`swift run mihomo-menubar` or running the built binary there). Inside `net system-proxy on`, `isatty(STDIN_FILENO)` therefore read `true`, routing the command into the *interactive* multi-service prompt (`NetworkSetup.swift`'s `readLine()` path) instead of the intended non-interactive fail-closed path (AI.md §3 convention #12). Nobody was actually typing into that inherited terminal on the CLI subprocess's behalf, so `readLine()` returned something outside `1...activeServices.count` (or `nil`), which the CLI correctly — but unhelpfully, for this caller — reported as `invalid selection — selected index out of range`.

**Fix:**
- `Sources/mihomo-menubar/CLIBridge.swift`: every subprocess now gets `process.standardInput = FileHandle.nullDevice`, making `isatty(STDIN_FILENO)` deterministically `false` in the child regardless of how the menu bar app itself was launched. This is the correct fix on its own terms too: this app always answers prompts via flags (`--yes`), never via stdin, so there's never a legitimate reason for a spawned `mihomo-cli` to read from a real terminal.
- With that fix alone, the ambiguous-interface case now fails with the CLI's real, intended message (`error: multiple active network services found — cannot select interactively in non-interactive context (fix: specify target service with --interface)`) instead of the confusing one above — but "Set as System Proxy" would still just fail outright for anyone with more than one active network service, since the menu never passed `--interface`.
- New `Sources/mihomo-menubar/NetworkServiceLister.swift`: read-only enumeration of `networksetup -listallnetworkservices` output (strips the header line and the `*`-disabled-service marker). Deliberately does not duplicate `NetworkSetup.getActiveServices()`'s activity-detection logic (§4.1.3-style `-getinfo` link/address checking) — it only lists names for the user to choose from; `mihomo-cli` still makes every actual correctness decision when the chosen `--interface` is applied.
- `StatusItemController.toggleSystemProxy()`: on the ambiguous-interface failure specifically (detected by matching CLIError's `"multiple active network services"` text), presents an `NSAlert` with an `NSPopUpButton` of all listed services and retries `net system-proxy on --interface <chosen> --yes` — the GUI equivalent of the CLI's own terminal prompt, instead of a dead end.

**Testing:** none — same toolchain caveat as the original menu bar implementation (CHANGELOG entry 29); reasoned through against `NetworkSetup.swift`'s existing, tested prompt logic and the exact error text it produces, but not run against a real Mac with multiple active network services.
