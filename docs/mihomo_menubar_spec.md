...
| Set as System Proxy (on) | `mihomo net system-proxy on --yes` | `--yes` auto-confirms the "proxy already set by another app" overwrite warning. No `--interface` is passed up front. If the CLI reports the ambiguous-multiple-active-services case, the app falls back to an `NSAlert` + popup listing every service from `networksetup -listallnetworkservices` (read-only enumeration, no activity judgment — see `NetworkServiceLister.swift`) and retries with the chosen `--interface`, mirroring the CLI's own terminal prompt for the same case. |
