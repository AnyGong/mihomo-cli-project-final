...
#
# UPDATE (menubar support + extended smoke tests):
#   - Explicitly builds the `mihomo-menubar` product (if present in Package.swift)
#   - Expands home-directory shim to cover FileManager.default.homeDirectoryForCurrentUser
#   - Adds smoke tests for `mode status --json`, `net status --json`, `daemon status --json`
#   - Adds a non-interactive `net system-proxy on` check to ensure no TTY hang
#
# Usage:
...
cleanup() {
  ...
  if [ "${#PATCHED_FILES[@]}" -gt 0 ]; then
    ...
    if git -C "$REPO_ROOT" checkout -- "${PATCHED_FILES[@]}" 2>>"$LOG_DIR/cleanup.log"; then
      log "Reverted home-directory patch in: ${PATCHED_FILES[*]}"
    else
...
#    Also covers FileManager.default.homeDirectoryForCurrentUser.
# ---------------------------------------------------------------------
...
# Replace NSHomeDirectory() and FileManager.default.homeDirectoryForCurrentUser
# with testableHomeDirectory() in all source files except the shim itself.
# This ensures all home-based storage paths (including FileManager APIs)
# are redirected to the sandbox during tests.
while IFS= read -r full; do
  [ "$full" = "$HOME_SHIM_FILE" ] && continue
  # Replace NSHomeDirectory()
  sed -i '' 's/NSHomeDirectory()/testableHomeDirectory()/g' "$full"
  # Replace FileManager.default.homeDirectoryForCurrentUser (common pattern)
  sed -i '' 's/FileManager\.default\.homeDirectoryForCurrentUser\.path/testableHomeDirectory()/g' "$full"
  # Also replace any direct use of homeDirectoryForCurrentUser URL (unlikely but safe)
  sed -i '' 's/FileManager\.default\.homeDirectoryForCurrentUser/testableHomeDirectory()/g' "$full"
  rel="${full#"$REPO_ROOT"/}"
  PATCHED_FILES+=("$rel")
done < <(grep -rl "NSHomeDirectory()\|FileManager\.default\.homeDirectoryForCurrentUser" "$SRC_DIR" 2>/dev/null)

log "Patched files (reverted at end of run): ${PATCHED_FILES[*]:-none}"
if [ "${#PATCHED_FILES[@]}" -eq 0 ]; then
  log "WARNING: no home-directory usage found at all — storage paths may not be sandboxed. Investigate before trusting this run's log/audit/doctor output as isolated."
fi
...
# Explicitly build mihomo-menubar product if present in Package.swift
if grep -q "mihomo-menubar" "$PKG_DIR/Package.swift"; then
  section "swift build --product mihomo-menubar"
  ( cd "$PKG_DIR" && swift build --product mihomo-menubar 2>&1 ) | tee "$LOG_DIR/menubar-build.log"
  MENUBAR_BUILD_OK=${PIPESTATUS[0]}
  log "mihomo-menubar build exit code: $MENUBAR_BUILD_OK"
  if [ "$MENUBAR_BUILD_OK" -ne 0 ]; then
    log "WARNING: mihomo-menubar build failed (but main build succeeded). See menubar-build.log."
  fi
else
  log "mihomo-menubar product not found in Package.swift — skipping explicit menubar build."
fi
...
# ---------------------------------------------------------------------
# Additional smoke tests for recently implemented command surfaces
# ---------------------------------------------------------------------
section "ADDITIONAL SMOKE TESTS (mode/net/daemon status --json)"

# mode status --json should exit 0 and output JSON
out="$(run_cli "mode status --json" mode status --json)"; ec=$?
check "mode status --json" $ec 0
if grep -q '"mode"' <<<"$out"; then
  log "  (confirmed mode status JSON contains 'mode' field)"
else
  log "  WARNING: mode status --json output did not contain expected field, but exit code was 0"
fi

# net status --json should exit 0 and output JSON
out="$(run_cli "net status --json" net status --json)"; ec=$?
check "net status --json" $ec 0
if grep -q '"tun"' <<<"$out"; then
  log "  (confirmed net status JSON contains 'tun' field)"
else
  log "  WARNING: net status --json output did not contain expected field, but exit code was 0"
fi

# daemon status --json may exit 0 if daemon not installed, or 1 if not running.
# We only care that it does not crash and returns something.
out="$(run_cli "daemon status --json" daemon status --json)"; ec=$?
if [ "$ec" -gt 128 ]; then
  log "CRASH: daemon status --json looks like a crash (exit $ec)"; FAIL=$((FAIL+1))
else
  log "INFO: daemon status --json -> exit $ec (non-crash, acceptable)"
fi

# Non-interactive net system-proxy on should not hang and should return an error
# if multiple interfaces are active (or 0 if single interface and --yes given).
# We ensure it returns quickly and does not hang by setting a timeout.
log "Testing non-interactive net system-proxy on (should not hang)..."
out="$(timeout 10 "$BIN_PATH" net system-proxy on --yes 2>&1)"; ec=$?
if [ "$ec" -eq 124 ]; then
  log "FAIL: net system-proxy on --yes hung (timeout)"; FAIL=$((FAIL+1))
elif [ "$ec" -gt 128 ]; then
  log "CRASH: net system-proxy on --yes crashed (exit $ec)"; FAIL=$((FAIL+1))
else
  log "PASS: net system-proxy on --yes completed with exit $ec (output: ${out:0:80})"
fi
...
if grep -q "mihomo-menubar" "$PKG_DIR/Package.swift"; then
  log "mihomo-menubar build: $([ "$MENUBAR_BUILD_OK" -eq 0 ] && echo OK || echo FAILED) (see menubar-build.log)"
fi
